...
class OnlineLearner(ABC):
    ...
    def __init__(self, loss, T, model_list, partition=None, **kwargs):
        ...
        # Create regret_loss partial, pre-populated with partition information
    ...
    @abstractmethod
    def update(self, X, hint=None, X_fb=None, y_fb=None, w_fb=None, **kwargs): 
        ...
        Args:
            ...
            X_fb (np.array): matrix of model predictions at feedback time, X_{t-D} 
...
class AdaHedgeD(OnlineExpert):
    """
    AdaHedgeD module implements delayed AdaHedge online learning algorithm.
    """    
    def __init__(self, loss, T, model_list, partition=None, reg="adahedged"):
        """ Initializes online_expert 

        Args:
           reg: regularization strategy [ "dub | "adahedged" ]
        """                
        ...
        if reg not in supported_reg:
            raise ValueError(f"Unsupported regularizer for AdaHedgeD {reg}.")
    ...
    def reset_alg_params(self, T):
        ...
        self.hint_prev = np.zeros(self.w.shape) # pervious hint
        self.lam = 0.0 # time varying regularization, lam_0
    ...
    def get_at_bound(self, g_fb, g_os, hint_t):
        """ Get bound on the value of a_t terms, if diam(W) = 2

        Args:
            g_fb: (d,) numpy array, most recent feedback gradient t-D
            g_os: (d,) numpy array, sum of gradients outstanding at time t-D
            hint_t: (d,) numpy array, hint at t-D
        """
        g_norm = np.linalg.norm(g_fb, ord=np.inf)
        err_norm = np.linalg.norm(hint_t - g_os, ord=np.inf)
        return 2*np.min([g_norm, err_norm])

    def get_bt_bound(self, g_fb, g_os, hint_t):
        """ Get bound on the value of b_t terms, if diam(W) = 2
            g_fb: (d,) numpy array, most recent feedback gradient t-D
            g_os: (d,) numpy array, sum of gradients outstanding at time t-D
            hint_t: (d,) numpy array, hint at t-D
        """
    ...
    def get_reg_uniform(self, g_fb, w_fb, **params):
        ...
        D = params.get('D', 0)
        d = g_fb.shape[0]
        g_os = params.get('g_os', np.zeros(d,))
        hint_t = params.get('hint_t', np.zeros(d,))

        # Get a_t and b_t upper bounds
        a_t = self.get_at_bound(g_fb, g_os, hint_t)
        b_t = self.get_bt_bound(g_fb, g_os, hint_t)
    ...
    def get_reg_adahedge(self, g_fb, w_fb, **params):
        """ Returns the delayed optimistic foresight gains.  
            g_fb: (d,) numpy array, most recent feedback gradient t-D
            w_fb: (d,) numpy array, play at time t-D
        ...
        # Get parameters
        lam_fb = params.get('lam_t', 0.0)
        g_os = params.get('g_os', np.zeros(self.d,))
        hint_t = params.get('hint_t', np.zeros(self.d,))

        # Get delta value
        delta = self.get_delta(g_fb, w_fb, lam_fb, hint_t, g_os)
    ...
    def get_delta(self, g_fb, w_fb, lam_fb, hint_fb, g_os):
        """ Returns the delayed optimistic foresight gains. 
            ...
            w_fb: (d,) numpy array, play at time t-D
            lam_fb: (float) value of regularizer at t-D
            hint_fb: (d,) numpy array, hint at t-D
            g_os: (d,) numpy array, sum of gradients outstanding at time t-D
        """
        
        # Compute the Be-The-Regularized-Leader Solution
        if np.isclose(lam_fb, 0):
            w_i =  (self.theta  == self.theta.max()) # Set to the best expert so far
            w_btrl = np.zeros((self.d,))
            w_btrl[w_i] = 1.0  / np.sum(w_i)
        else:
            maxv = np.max(self.theta)
            w_btrl =  np.exp((self.theta -  maxv) / lam_fb) 
            w_btrl = w_btrl / np.sum(w_btrl, axis=None)
        ...
        g_diff = hint_fb - g_os
        shift = np.max(g_diff[w_fb != 0.0])

        if np.isclose(lam_fb, 0):
            delta_aux = -np.dot(g_diff - shift, w_fb)
        else:
            delta_aux = lam_fb * \
                np.log(np.sum(w_fb * np.exp((g_diff - shift) / lam_fb))) - \
                np.dot(g_diff - shift, w_fb)
    ...
    def update_and_predict(self, X_cur, hint=None, X_fb=None, y_fb=None, w_fb=None, **params): 
        ...
        if self.update_learner:
            ...
            if self.reg == "adaptive":
                self.lam, self.delta  = self.get_reg_adahedge(g_fb, w_fb, **params)
            elif self.reg == "upper_bound":
                self.lam, self.delta  = self.get_reg_uniform(g_fb, w_fb, **params)
